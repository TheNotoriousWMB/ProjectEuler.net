...
public class main {
	...
	public static void main(String[] args) {
		...
		// Check Vertical
		for (i = 0; i < DIMEN - (ADJ - 1); ++i)
			for (j = 0; j < DIMEN; ++j) {
				prod = (grid[i][j] * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j]);

				if (prod > max) {
					adj = new int[] { grid[i][j], grid[i + 1][j],
		...
		// Check Horizontal
		for (i = 0; i < DIMEN; ++i)
			for (j = 0; j < DIMEN - (ADJ - 1); ++j) {
				prod = (grid[i][j] * grid[i][j + 1] * grid[i][j + 2] * grid[i][j + 3]);

				if (prod > max) {
					adj = new int[] { grid[i][j], grid[i][j + 1],
		...
		for (i = 0; i < DIMEN - (ADJ - 1); ++i)
			for (j = 0; j < DIMEN - (ADJ - 1); ++j) {
				...
				if (prod > max) {
					adj = new int[] { grid[i][j], grid[i + 1][j + 1],
		...
		System.out.println(adj[ADJ - 1] + " = " + max);
	}
}
